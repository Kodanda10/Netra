...
export const SourcesSidePanel: React.FC<{ open:boolean; onClose:()=>void; sources:{source:string;count:number}[]; order?: string[] }>
  = ({ open, onClose, sources, order }) => {
  React.useEffect(() => {
    if (!open) return
        ...
        <div className="fixed inset-0 z-50" aria-modal="true" role="dialog">
          ...
          <motion.div layoutId={layoutIds.sourcePanel} className="absolute top-0 left-0 h-full w-[380px] glass-liquid rounded-r-3xl bg-transparent p-4 overflow-y-auto"
            ...
            <div className="space-y-1">
              {(order && order.length ? order.map(k => sources.find(s=>s.source===k)!).filter(Boolean) : sources).map(s => (
